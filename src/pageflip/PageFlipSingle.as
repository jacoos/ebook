...
package src.pageflip
{
	...
	public class PageFlipSingle {
		...
		public var onPageStart:Function = null;//翻页动作结束时调用
		...
		//**加载优化（modify by shine）------------------------------------------------------------------------
		private var nowLoadIndex:int;
		private var loadIndexArray:Array;
		...
		{
			//$point：鼠标位置   $limitPoint：圆心    $limitGap：半径
		...
		private function SetLoadMC():void {
			...
			nowLoadIndex = 1;
			loadIndexArray = new Array();

			for (var i:Number = 1; i<=book_totalpage; i++) {
				loadIndexArray.push(i);
				...
				createPreLoadingMC(book_root["pload_" + i]);
			}
			
			loadPageFile();
		}
		
		private function createPreLoadingMC(targetPage:MovieClip)
		{
			targetPage.graphics.beginFill(0xffffff);
			targetPage.graphics.lineTo(0, book_height);
			targetPage.graphics.lineTo(book_width, book_height);
			targetPage.graphics.lineTo(book_width, 0);
			targetPage.graphics.lineTo(0, 0);
			targetPage.graphics.endFill();
			
			var preloading_txt:TextField = new TextField();
			preloading_txt.autoSize = TextFieldAutoSize.LEFT;
			preloading_txt.selectable = false;
			preloading_txt.textColor = 0x999999;
			preloading_txt.text = "loading...";
			
			preloading_txt.x = (book_width - preloading_txt.width) / 2;
			preloading_txt.y = (book_height-preloading_txt.height) / 2;
			
			targetPage.addChild(preloading_txt);
			
		}
		
		private function loadPageFile()
		{
			...
			var i:int = loadIndexArray[0];
			
			pageRequest = new URLRequest(docpath + myXML.child("page")[i - 1].attribute("src"));
			...
			loadIndexArray.shift();
			nowLoadIndex++;
		}
		function UpdatePreLoad()
		{
			var p_arr_index:int = loadIndexArray.indexOf(book_page);
			var f_arr:Array = loadIndexArray.splice(p_arr_index, loadIndexArray.length - p_arr_index);
			var b_arr:Array = loadIndexArray.splice(0, loadIndexArray.length);
			loadIndexArray = f_arr.concat(b_arr);
		...
		private function SetPageMC(pageNum:Number):void {
			...
			TweenLite.to(p_mc1, 0.5, { alpha:0 } );
		...
		/*	var p_mc:MovieClip = new MovieClip();
			...
			if (pageNum+1 > 0 && pageNum+1 <= book_totalpage) 
			{
				p_mc=book_root["pload_"+(pageNum+1)];
			}
			p_mc.x = book_width; 
			p_mc.y = 0;
			pageMC.addChild(p_mc);
			
			if (p_mc.loadedflag == true && p_mc.loadedtype == "application/x-shockwave-flash") 
			{
				MC_content=p_mc["loader"].content;
				MC_content.gotoAndPlay(2);
			}
		*/	//------------end-------------------
		}
		//End Setting------------------------------------------------------------------------
		...
		private function LoadEnd(evtObj:Event):void {
			//------sj---------加载完毕后改变页面大小-------
			var content:DisplayObject = evtObj.target.loader as DisplayObject;
			if (content.width < content.height)//仅当竖版时生效。因为还有两面为一页的情况。
			{
				content.width = book_width;
				content.height = book_height;
			}
			else
			{
				content.width = book_width*2;
				content.height = book_height;
			}
			//------end---------
			var obj=content.parent;
		//	var obj=evtObj.target.loader.parent;
			...
			if(nowLoadIndex<=book_totalpage){
				loadPageFile();
			}
